...
import { ArrowRight, CheckCircle2, ChevronDown, Globe, Menu, X } from "lucide-react";
...
export function SiteChrome({ children }: { children: ReactNode }) {
  ...
  return (
    <ModalCtx.Provider value={open}>
      ...
      ))}</div><div className="nav-actions"><LangToggle /><button className="button secondary" onClick={() => open("demo")}>{t("Book Demo", "Réserver une démo")}</button><button className="button primary" onClick={() => open("trial")}>{t("Start Free Trial", "Essai gratuit")}</button></div><button className="menu-button" aria-label="Toggle menu" aria-expanded={mobileOpen} onClick={() => setMobileOpen(!mobileOpen)}>{mobileOpen ? <X /> : <Menu />}</button></div></nav>{mobileOpen && <div className="mobile-menu"><LangToggle />{NAV.map((e) => e.items ? (
