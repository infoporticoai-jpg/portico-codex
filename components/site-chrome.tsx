...
function DemoModal({ onClose }: { onClose: () => void }) {
      ...
      const res = await fetch("/api/lead", {
        ...
        body: JSON.stringify({
          ...
          fields: {
            ...
            callsPerMonth: data.get("callsPerMonth"),
            goal: data.get("goal"),
          },
        }),
      });
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="contact-title" onMouseDown={e => e.stopPropagation()}>
    ...
    {sent ? <div className="success"><CheckCircle2 size={38} /><h2>{t("You’re all set.", "Tout est prêt.")}</h2><p>{t("Thanks for your interest. We’ll be in touch shortly.", "Merci de votre intérêt. Nous vous contacterons sous peu.")}</p><button className="button primary" onClick={onClose}>{t("Close", "Fermer")}</button></div> : <><span className="eyebrow">{t("Let’s talk", "Discutons")}</span><h2 id="contact-title">{t("Book an enterprise demo.", "Réservez une démo entreprise.")}</h2><p>{t("Share a few details and our team will tailor the conversation to your operation.", "Partagez quelques détails et notre équipe adaptera la conversation à votre entreprise.")}</p><form onSubmit={submit}><label>{t("Full name", "Nom complet")}<input required name="name" placeholder={t("Jordan Lee", "Jordan Lee")} /></label><label>{t("Work email", "Courriel professionnel")}<input required type="email" name="email" placeholder="you@company.com" /></label><label>{t("Company name", "Nom de l’entreprise")}<input required name="company" placeholder={t("Your company", "Votre entreprise")} /></label><label>{t("Location", "Emplacement")}<input name="location" placeholder={t("Montreal, QC", "Montréal, QC")} /></label><label>{t("Team size", "Taille de l’équipe")}<select name="teamSize" defaultValue=""><option value="" disabled>{t("Select a range", "Choisissez une plage")}</option><option>1–10</option><option>11–50</option><option>51–250</option><option>251+</option></select></label><label>{t("Calls per month", "Appels par mois")}<select name="callsPerMonth" defaultValue=""><option value="" disabled>{t("Select a range", "Choisissez une plage")}</option><option>{t("Under 50", "Moins de 50")}</option><option>50–200</option><option>200–500</option><option>500–1,000</option><option>{t("1,000+", "1 000+")}</option></select></label><label>{t("What would make this demo a win for you?", "Qu’est-ce qui ferait de cette démo une réussite pour vous?")}<textarea name="goal" rows={3} placeholder={t("e.g. seeing how it handles after-hours emergency calls", "ex. voir comment il gère les appels d’urgence après les heures")} /></label>{failed && <p className="form-error">{t("Something went wrong sending that — please try again.", "Une erreur est survenue lors de l’envoi — veuillez réessayer.")}</p>}<button className="button primary" type="submit" disabled={sending}>{sending ? t("Sending…", "Envoi…") : t("Request demo", "Demander la démo")}<ArrowRight size={16} /></button></form></>}
  </section></div>;
}
