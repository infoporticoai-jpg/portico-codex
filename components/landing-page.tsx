...
export function LandingPage() {
  ...
  const { t } = useLang();
  return <>
    <section className="hero"><div className="shell"><span className="eyebrow reveal">{t("Voice agents · Human backup", "Agents vocaux · Renfort humain")}</span><h1 className="reveal">{t("Never Lose a Customer", "Ne perdez plus jamais")}<br />{t("to a Missed Call.", "un client à cause d’un appel manqué.")}</h1><p className="reveal">{t("Portico turns every incoming call into a booked appointment or a qualified lead — during the day, after hours, and every minute your team is busy.", "Portico transforme chaque appel entrant en rendez-vous réservé ou en prospect qualifié — le jour, après les heures, et à tout moment où votre équipe est occupée.")}</p><div className="hero-actions reveal"><button className="button primary" onClick={() => open("trial")}>{t("Start Free Trial", "Essai gratuit")} <ArrowRight size={16} /></button><button className="button secondary" onClick={() => open("demo")}>{t("Book Demo", "Réserver une démo")}</button></div><div className="fine reveal">{t("Free 14-day trial on self-serve plans · Up to 30 voice-agent handled calls included", "Essai gratuit de 14 jours sur les forfaits libre-service · Jusqu’à 30 appels gérés par agent vocal inclus")} · <a href="/voice-agents">{t("See everything Voice Agents can do →", "Découvrez tout ce que les agents vocaux peuvent faire →")}</a></div><div className="reveal"><VoiceDemo onBook={() => open("demo")} /></div></div></section>
